...
class controller {
    ...
    /*устанавливаем масив который будет идти сразу без Index*/
    public function setNoIndexController($arr)
    {
        $this->_arrayNoIndex=$arr;
    }
	...
	public function getView($sub=null){
        ...
        {
            if(in_array($this->_controller,$this->_arrayNoIndex))
            {
                $filename = $this -> _controller."Index.phtml";
            }
            else
            $filename = $this -> _controller . mb_strtoupper($this -> _action[0]) . mb_substr($this -> _action,1) . ".phtml";
        }
		if(!empty($sub))
		{
