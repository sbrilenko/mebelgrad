...
if (isset($login) && isset($pass)) {
    ...
    if ($db -> getCount() > 0) {
        ...
        // Проверка пароля
        if ($pass != $myrow['pass']) {
            Header ("Location: in.php?errorLogin=3");
            exit;
        }
        // Запускаем сессию
        ...
        Header ("Location: in.php?errorLogin=4");
        exit;
    }
} 
else 
    if (isset($login) && !isset($pass)) {
        Header ("Location: in.php?errorLogin=2");
        exit;
    }
    else
        if (!isset($login) && isset($pass)) {
            Header ("Location: in.php?errorLogin=1");
            exit;
        }
        else {
            Header ("Location: in.php?errorLogin=0");
            exit;
        }
?>