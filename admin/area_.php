  ...
  if(!empty($_GET['pomesh']) AND ($_GET['pomesh']!=-1) AND ($floor==1 AND $est_v_base==true) OR ($floor==2 AND $est_v_base==true) OR ($floor==3 AND $est_v_base==true))
			  {
				...
				////////фирма орендатора
				  $sql='SELECT * FROM contragents WHERE room_number="'.$pomeshenie.'"';
				  $db->query($sql);
				   echo "<div class='float_left preview_text'>Фирма орендатора:</div>";
				  if($db->getCount()>0)
				  {
				  	 $arr=$db->getArray();	
					 echo "<div class='float_left' style='line-height:25px;'>".$arr[0]['fio']."</div>";
				 ...
				 if($db->getCount()>0)
				  {
					 ...
					 echo "<div class='float_left' style='line-height:25px;'>".$arr[0]['contract_number']."</div>";
				  }
				  ///акт приема передачи
